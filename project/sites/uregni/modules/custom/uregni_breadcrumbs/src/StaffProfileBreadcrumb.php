 ...
 * {@inheritdoc}
 */
class StaffProfileBreadcrumb implements BreadcrumbBuilderInterface {
  ...
  public function applies(RouteMatchInterface $route_match) {
    ...
    if ($route_name == 'entity.node.canonical') {
      ...
      if (!empty($this->node)) {
        if ($this->node->bundle() == 'uregni_staff_member') {
          $match = TRUE;
        }
      }
    }
  ...
  public function build(RouteMatchInterface $route_match) {
    ...
    if ($this->node) {
      ...
      $links[] = Link::fromTextandUrl(t('Staff profile'), Url::fromUri('internal:/staff-profiles'));
