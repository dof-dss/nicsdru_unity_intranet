 ...
 * {@inheritdoc}
 */
class LegalBreadcrumb implements BreadcrumbBuilderInterface {
  ...
  public function applies(RouteMatchInterface $route_match) {
    ...
    if ($route_name == 'entity.node.canonical') {
      ...
      if (!empty($this->node)) {
        if ($this->node->bundle() == 'legal_guidance') {
          $match = TRUE;
        }
      }
    }
  ...
  public function build(RouteMatchInterface $route_match) {
    ...
    if ($this->node) {
      ...
      $links[] = Link::fromTextandUrl(t('Legal guidance'), Url::fromRoute('view.legal_search.legal_search_page'));
